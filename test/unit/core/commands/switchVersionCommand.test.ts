/**
 * Switch Version Command Tests
 * Tests for switching between story versions
 */

import { SwitchVersionCommand } from '../../../../src/core/commands/switchVersionCommand';
...
describe('SwitchVersionCommand', () => {
  let command: SwitchVersionCommand;
  let storyRepository: StoryRepository;
  let versionRepository: VersionRepository;
  let gitRepository: GitRepository;
  let versionSwitcher: VersionSwitcher;
  ...
  beforeEach(async () => {
    ...
    testRepoPath = path.join('/tmp', `babel-switch-version-test-${timestamp}-${testId}`);
    ...
    versionSwitcher = new VersionSwitcher(gitRepository);
    ...
    command = new SwitchVersionCommand(versionRepository, versionSwitcher, storyRepository);
  });
  ...
  describe('execute', () => {
    ...
    it('should switch to selected version', async () => {
      // Create test story and version
      const story = storyManager.createStory('Test Story', StoryType.NOVEL);
      versionRepository.create({
        id: `v1-${Date.now()}`,
    ...
    it('should list default version when story is created', async () => {
      // Create story - automatically gets 'main' as default version
      const story = storyManager.createStory('Story with Default', StoryType.NOVEL);

      // Create the main branch in git
      await gitRepository.createBranch('main');

      (vscode.window.showQuickPick as any).mockResolvedValueOnce({
        label: 'Story with Default',
        storyId: story.id,
      });
      (vscode.window.showQuickPick as any).mockResolvedValueOnce({
        label: 'main',
      ...
      expect(result.success).toBe(true);
      expect((result.data as any)?.branchName).toBe('main');
    });
    ...
    it('should show success message', async () => {
      ...
      // Create the branch in git
      await gitRepository.createBranch('release-1.0');
