  ...
  drawGlobalOverview: function() {
    ...
    var item_block = calendarHeatmap.items.selectAll('.item-block-year')
      ...
      .on('mouseover', function(d) {
        ...
        // Construct tooltip
        var tooltip_html = '';
        tooltip_html += '<div><span><strong>Total words written:</strong></span>';

        var words = parseInt(d.total, 10);
        if (words > 0) {
          tooltip_html += '<span>' + (words === 1 ? '1 word' : words + ' words') + '</span></div>';
        }
        tooltip_html += '<br />';
  ...
  drawYearOverview: function() {
    ...
    calendarHeatmap.items.selectAll('.item-circle')
      ...
      .on('mouseover', function(d) {
        ...
        // Construct tooltip
        var tooltip_html = '';
        tooltip_html += '<div class="header"><strong>' + (d.total ? d.total : 'No words') + ' words</strong></div>';
  ...
  drawMonthOverview: function() {
    ...
    item_block.selectAll('.item-block-rect')
      ...
      .on('mouseover', function(d) {
        ...
        tooltip_html += '<div><strong>' + (d.value ? d.value : 'No activity') + ' words.</strong></div>';
